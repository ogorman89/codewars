...
function sortThePile(pileOfTowels, weeklyUsedTowels) {
    ...
    for (let i = 0; i < weeklyUsedTowels.length; i++){
      usedTowels = pileOfTowels.slice(pileOfTowels.length-weeklyUsedTowels[i]).sort().reverse();
      pileOfTowels = pileOfTowels.slice(0,pileOfTowels.length-weeklyUsedTowels[i]).concat(usedTowels);
    }
    return pileOfTowels
  }